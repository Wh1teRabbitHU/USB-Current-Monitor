/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2023 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g0xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define STATUS_LED_Pin GPIO_PIN_0
#define STATUS_LED_GPIO_Port GPIOA
#define ERROR_LED_Pin GPIO_PIN_1
#define ERROR_LED_GPIO_Port GPIOA
#define DISPLAY_DC_Pin GPIO_PIN_2
#define DISPLAY_DC_GPIO_Port GPIOA
#define DISPLAY_CS_Pin GPIO_PIN_3
#define DISPLAY_CS_GPIO_Port GPIOA
#define DISPLAY_SCL_Pin GPIO_PIN_4
#define DISPLAY_SCL_GPIO_Port GPIOA
#define DISPLAY_SDA_Pin GPIO_PIN_5
#define DISPLAY_SDA_GPIO_Port GPIOA
#define DISPLAY_RESET_Pin GPIO_PIN_6
#define DISPLAY_RESET_GPIO_Port GPIOA
#define DISPLAY_BLACKLIGHT_EN_Pin GPIO_PIN_7
#define DISPLAY_BLACKLIGHT_EN_GPIO_Port GPIOA
#define PATH_1_EN_Pin GPIO_PIN_9
#define PATH_1_EN_GPIO_Port GPIOA
#define PATH_2_EN_Pin GPIO_PIN_10
#define PATH_2_EN_GPIO_Port GPIOA
#define MUX_A0_Pin GPIO_PIN_11
#define MUX_A0_GPIO_Port GPIOA
#define MUX_A1_Pin GPIO_PIN_12
#define MUX_A1_GPIO_Port GPIOA

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
